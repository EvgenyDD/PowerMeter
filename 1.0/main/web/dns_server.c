...
#define SERVER_HOSTNAME "web.server"
...
static char *parse_dns_name(char *raw_name, char *parsed_name, size_t parsed_name_max_len)
{
	...
	do
	{
		...
		name_len += (sub_name_len + 1); // (len + 1) since we are adding  a '.'
		if(name_len > parsed_name_max_len) return NULL;

		memcpy(name_itr, label + 1, sub_name_len); // Copy the sub name that follows the the label
		name_itr[sub_name_len] = '.';
		name_itr += (sub_name_len + 1);
		label += sub_name_len + 1;
	} while(*label != 0);

	parsed_name[name_len - 1] = '\0'; // Terminate the final string, replacing the last '.'
	return label + 1;				  // Return pointer to first char after the name
}
...
static int parse_dns_request(char *req, size_t req_len, char *dns_reply, size_t dns_reply_max_len)
{
	...
	// ESP_LOGI("DNS", "DNS query with header id: 0x%X, flags: 0x%X, qd_count: %d", ntohs(header->id), ntohs(header->flags), ntohs(header->qd_count));

	if((header->flags & OPCODE_MASK) != 0) return 0; // Not a standard query

	header->flags |= QR_FLAG; // Set question response flag
	...
	char *cur_ans_ptr = dns_reply + req_len; // Pointer to current answer and question
	char *cur_qd_ptr = dns_reply + sizeof(dns_header_t);
	char name[128];

	for(int i = 0; i < qd_count; i++) // Respond to all questions with the ESP32's IP address
	{
		...
		// ESP_LOGI("DNS", "Received type: %d | Class: %d | Question for: %s", qd_type, qd_class, name);

		// if((qd_type == QD_TYPE_A || qd_type == 28) && strcasecmp(name, SERVER_HOSTNAME) == 0)
		if(strcasecmp(name, SERVER_HOSTNAME) == 0)
		{
			...
			ESP_LOGI("DNS", "Answer (for %s) with PTR offset: 0x%X and IP 0x%lX", name, ntohs(answer->ptr_offset), ip_info.ip.addr);
		...
		{
			// ESP_LOGI("DNS", "Req %s %d", name, qd_type);
		}
	}
	return reply_len;
}
...
void dns_server_task(void *pvParameters)
{
	...
	while(1)
	{
		...
		if(err < 0) ESP_LOGE("DNS", "Socket unable to bind: errno %d", errno);
		...
		while(1)
		{
			...
			{
				if(source_addr.sin6_family == PF_INET) // Get the sender's ip address as string
				{
